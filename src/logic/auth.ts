...
type GametimesUserData = Pick<Osdk<GametimesUser>, "id" | "emailVerifiedAt" | "avatarUrl" | "name"> & {
    email: string;
};
...
const foundryAdapter: Adapter = {
    ...
    updateUser: async (user) => {
        ...
        const patchedUser = {
            ...
            emailVerifiedAt: user.emailVerified?.toISOString() ?? foundryUser.emailVerifiedAt,
    ...
    getUserByEmail: async (email) => {
        const user = await foundryClient(GametimesUser).where({ email }).fetchPage({ $pageSize: 1 });
        return user.data.length < 1 ? null : toAdapterUser(user.data[0] as GametimesUserData);
    },
    ...
    getUserByAccount: async (account) => {
        const user = await foundryClient(GametimesUser)
            .where({
                $and: [{ provider: account.provider }, { providerAccountId: account.providerAccountId }],
            })
